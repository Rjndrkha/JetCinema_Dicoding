package com.dicoding.rajendra.jettixid.data

import com.dicoding.rajendra.jettixid.model.FakeMovieDataSource
...
class RewardRepository {
    ...
    fun updateOrderReward(rewardId: Long, newCountValue: Int): Flow<Boolean> {
        val index = orderRewards.indexOfFirst { it.movie.id == rewardId }
        val result = if (index >= 0) {
            val orderReward = orderRewards[index]
            orderRewards[index] =
                orderReward.copy(movie = orderReward.movie, count = newCountValue)
            true
        } else {
            false
        }
